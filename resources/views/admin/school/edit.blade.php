...
<div class="col-12 grid-margin stretch-card">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('updatesch',$School->id) }}" id="editstudent" method="POST" enctype="multipart/form-data" class="forms-sample">
                @csrf
                <div class="row">
                    <div class="form-group col-md-6 col-sm-12">
                        <label for="username">School Name<span class="mandatory-symbol">*</span></label>
                        <input type="text" Name="name" class="form-control {{ $errors->has('Student_Name') ? 'is-invalid' : '' }}" id="exampleInputName1" placeholder="Student Name" value="{{ $School->name }}">
                        @error('name')
                    ...
                    <div class="form-group col-md-6 col-sm-12">
                        ...
                        <textarea type="text" Name="address" class="form-control {{ $errors->has('Student_Name') ? 'is-invalid' : '' }}" id="exampleInputName1" placeholder="Student Name">{{ $School->description }}</textarea>
                        @error('address')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
